get_cloud_db <- function(
    db_filename = "my_database.db",
    project_dirname = "Prono_PL",
    key_windows = "C:/Users/vmoui/Documents/api_keys/",
    key_linux = "/home/vmouillot/Documents/api_keys/"
) {
  # Choisir le dossier contenant la clé selon OS
  key_path <- if (.Platform$OS.type == "windows") key_windows else key_linux
  key_file <- list.files(key_path, full.names = TRUE)[1]
  
  # Trouver la racine du projet
  root <- rprojroot::find_root(rprojroot::has_dir(project_dirname))
  
  # Authentification Google
  googledrive::drive_auth(path = key_file)
  
  # Télécharger le fichier depuis Google Drive
  file <- googledrive::drive_get(db_filename)
  db_local_path <- file.path(root, project_dirname, db_filename)
  googledrive::drive_download(file, path = db_local_path, overwrite = TRUE)
  
  # Retourner le chemin vers le fichier
