...
get_nb_points_all_weeks <- function(df, side = NA_character_, nb_match = 3) {
  ...
  last_results_function <- function(df, side, nb_match, current_week) {
    df_result <- df %>%
      ...
      mutate(Result_team = case_when(
        ((result == "H") & (Side == "Home")) | 
          ((result == "A") & (Side == "Away")) ~ 3,
        ((result == "H") & (Side == "Away")) | 
          ((result == "A") & (Side == "Home")) ~ 0,
        TRUE ~ 1
      ))
    ...
    res <- df_result %>%
      ...
      pivot_wider(names_from = J_label, values_from = Result_team) # Calculer la somme des points
  ...
  final_results <- bind_rows(results_list) %>%
      ...
      mutate(nb_point = rowSums(across(starts_with("J-")), na.rm = TRUE)) %>% 
    select(Team_id, Wk, nb_point) # Remplacer les NAs par 1 (Draw)
  
  if (!is.na(side) && side != "") {
    names(final_results)[-c(1,2)] <- paste0(names(final_results)[-c(1,2)], "_", side)  # Ajouter le suffixe au nom des colonnes
  }
